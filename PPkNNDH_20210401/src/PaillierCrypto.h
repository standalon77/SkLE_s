...
//
//const int THREAD1_NUM  = 1;					// (check) Main thread의 개수.
...
typedef struct {
    ...
    paillier_ciphertext_t cData[DATA_NUM][DATA_DIMN];		// question: 객체로 된 멤버 변수를 동적생성하는 방법?? (new가 아닌 함수에 의해서 메모리 할당)
    paillier_ciphertext_t cClass[DATA_NUM];
} in_t;

// 모든 thread의 공유 데이터
typedef struct {
//	paillier_ciphertext_t cS[DATA_NUM];								// SSED result
//	paillier_ciphertext_t cSbit[DATA_NUM][DATA_SQUARE_LENGTH];		// SBD result
	paillier_ciphertext_t cDist[DATA_NUM];								// SSED result
	paillier_ciphertext_t cDisB[DATA_NUM][DATA_SQUARE_LENGTH];		// SBD result
//	paillier_ciphertext_t cRes[DATA_NUM];								// SkLE_s result
//	paillier_ciphertext_t cTCnt[THREAD1_NUM]; 						// (SkLE_s) thread cnt
//	paillier_ciphertext_t cCnt;					 						// (SkLE_s) total cnt
	paillier_ciphertext_t cK[DATA_NUM];								// (SkLE_s) result
	paillier_ciphertext_t cTS[THREAD1_NUM]; 							// (SkLE_s) thread cnt
	paillier_ciphertext_t cS;					 					// (SkLE_s) total cnt
//	paillier_ciphertext_t cCmp;					 						// (SkLE_s) comparison
//	int					  iCmp;					 						// (SkLE_s) comparison

//	paillier_ciphertext_t cTF[THREAD1_NUM][CLASS_SIZE];				// (SCF) thread result
//	paillier_ciphertext_t cF[CLASS_SIZE];								// SCF result
...
//	paillier_ciphertext_t cMc[CLASS_SIZE];								// computing mc_j
	paillier_ciphertext_t cTFre[THREAD1_NUM][CLASS_SIZE];				// (SCF) thread result
	paillier_ciphertext_t cFreq[CLASS_SIZE];							// SCF result
	paillier_ciphertext_t cFreB[CLASS_SIZE][DATA_NUMBER_LENGTH];	// SBD result for f'_j
	paillier_ciphertext_t cFK[CLASS_SIZE];								// SkLE_s result for f'_j
	paillier_ciphertext_t cRes[CLASS_SIZE];							// computing mc_j

	double dTime[7];
	time_t start;
	time_t end;

    int iRan[THREAD1_NUM+1];
    int iRan2[THREAD2_NUM+1];
} out_t;

// 자주 사용되는 상수값을 미리 계산 & 사용
typedef struct {
...
typedef struct {
	...
	paillier_ciphertext_t cC[THREAD_DATA_NUM];
	paillier_ciphertext_t cK[THREAD_DATA_NUM];
	paillier_ciphertext_t cU[THREAD_DATA_NUM];
} skle_e_t;

// SkLE에서 one thread 데이터
typedef struct {
	paillier_ciphertext_t cSb[2][DATA_NUMBER_LENGTH];		// Comparison 함수의 input parameter
	int 					 iKb	[DATA_NUMBER_LENGTH];		// Comparison 함수의 input parameter
...
//	paillier_ciphertext_t cSb[2][DATA_NUMBER_LENGTH];		// Comparison 함수의 input parameter
//	int 					 iKb	[DATA_NUMBER_LENGTH];		// Comparison 함수의 input parameter
////	paillier_ciphertext_t cTRes[THREAD_DATA_NUM];
////	paillier_ciphertext_t cIRes[THREAD_DATA_NUM];
////	paillier_ciphertext_t cCan[THREAD_DATA_NUM];
////	int iFoundBit[2];
//
//	paillier_ciphertext_t cC[THREAD_DATA_NUM];
//	paillier_ciphertext_t cK[THREAD_DATA_NUM];
...
//	int Comparison(paillier_ciphertext_t* cGamma, paillier_ciphertext_t* cS, 							  pre_t *tPre, unsigned short idx, thp_t* tRecv, th_t* tSend, unsigned char* ucSendPtr);
...
//	void SCF(out_t* tOut, in_t* tIn, 			pre_t* tPre, 					   unsigned short idx, thp_t* tRecv, th_t* tSend, unsigned char* ucSendPtr);
...
//	void SkLE_s_4	 (out_t* tOut, in_t *In, 	skle_t* tSkle,  				   unsigned short idx);
	void SkLE_s_4(out_t* tOut, skle_e_t* tSklee, skle_1_t* tSkle1, int bit, bool bFirst, unsigned short idx, thp_t* tRecv);
	void SkLE_s_5(out_t* tOut, skle_e_t* tSklee, bool bFirst, unsigned short idx);
	void SCF		 (out_t* tOut, in_t* tIn, 	pre_t* tPre, 			 unsigned short idx, thp_t* tRecv);
	void ComputeMc	 (out_t* tOut, in_t *In, 		   			 				   unsigned short idx);
