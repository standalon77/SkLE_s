...
void PaillierCrypto::PreComputation(pre_t *tPre, skle_t* tSkle, out_t *tOut, in_t* tIn, unsigned short idx)
{
	...
	for (int i=0 ; i<DATA_NUMBER_LENGTH ; i++) {
		#ifdef _DEBUG_INIT_1
		mpz_inits(tSkle->cSb[0][i].c, tSkle->cSb[1][i].c, NULL);
		#else
	...
	//?? thread(혼자/병렬에 따른 필요한 변수 개수)에 따라서 어떤 구조체로 갈지 고민하기.
	#ifdef _DEBUG_INIT_1
	mpz_inits(tSkle->cM.c, tSkle->cD.c, tSkle->cA.c, tSkle->cB.c, tSkle->cG.c, NULL);
	#else
	mpz_init2(tSkle->cM.c, GMP_N_SIZE+1);
	mpz_init2(tSkle->cD.c, GMP_N_SIZE+1);
	mpz_init2(tSkle->cA.c, GMP_N_SIZE+1);
	mpz_init2(tSkle->cB.c, GMP_N_SIZE+1);
	mpz_init2(tSkle->cG.c, GMP_N_SIZE+1);
	#endif
...
void PaillierCrypto::SecBitDecomp(out_t* tOut, skle_t* tSkle, pre_t* tPre, bool bFirst, unsigned short idx, thp_t* tRecv)
{
	...
	for (int i=iSrt ; i<iEnd ; i++) {
		...
		if (tSkle->bSkle)
			...
			cEXi = bFirst ? tOut->cDisB[i] : tOut->cFbit[i];
		...
		while (1) {
			...
			if (iGamma==1) {
				if (tSkle->bSkle) {
					for (int j=0 ; j<iLen ; j++)
						// E(x_j) = 1 - E(x_j)
						paillier_mul(mPubKey, &(tSkle->cSb[1][j]), &(tPre->c1), &_cEXi[j]);
...
//	cS = (tOut->cS)+idx;												// 각 thread의 S값
	...
	for (int i=0 ; i<iNum ; i++) {			// i = 각 thread의 data 시작점 ~ 종료점
		...
		cE = bFirst ? &(tOut->cDisB[SrtDat+i][bit]) : &(tOut->cFbit[SrtDat+i][bit]);
		cC = &(tSkle->cC[i]);
		cK = &(tSkle->cK[i]);
		...
		if (*pSyncCnt2 < THREAD1_NUM) {
			...
			paillier_ciphertext_t *cTotal_S = tOut->cTotal_S;

			// initialize cTotal_S
			mpz_set_ui(tOut->cTotal_S.c, 1);

			// S = Sum_(i=1~n) P[i]
			for (int i=0 ; i<iNumThread ; i++)
				paillier_mul(mPubKey, cTotal_S, cTotal_S, &(tOut->cS[i]));

			#ifdef _DEBUG_SkLE_s
			//DebugOut("S = Sum_(i=1~n) P[i]", tOut->cTotal_S.c, idx);
			DebugDec("S = Sum_(i=1~n) P[i] \t", cTotal_S, idx);
			#else
			//printf("%d \n", bit);
			printf("%d bit - ", bit);
			DebugDec("Cnt'\t\t", &(tOut->cTotal_S), idx);
			#endif

//			// temp code (delete)
//			mpz_set(tOut->cTotal_S.c, cS.c);
		...
		for (int i=0 ; i<iNum ; i++) {
			//iTidx = bFirst ? tOut->iRan[idx]+i  : tOut->iRan2[idx]+i;
			cE = bFirst ? &(tOut->cDisB[i][j]) : &(tOut->cFbit[i][j]);
			cC = &(tSkle->cC[i]);
			cK = &(tSkle->cK[i]);
...
void PaillierCrypto::SkLE_s_1(out_t* tOut, skle_t* tSkle, pre_t* tPre, int bit, bool bFirst, unsigned short idx, thp_t* tRecv)
{
	...
	for (int i=0 ; i<iNum ; i++) {			// i = 각 thread의 data 시작점 ~ 종료점
		...
		// S = Sum_(i=1~n) P[i]
		//paillier_mul(mPubKey, &(tOut->cS[idx]), &(tOut->cS[idx]), &(tSkle->cP[i]));
		paillier_mul(mPubKey, cS, cS, &cP);
		#ifdef _DEBUG_SkLE_s
		DebugDec("S = Sum_(i=1~n) P[i] \t", cS, idx);
		#endif
...
void PaillierCrypto::SkLE_s_234(out_t* tOut, skle_t* tSkle, pre_t* tPre, int bit, bool bFirst, unsigned short idx, thp_t* tRecv)
{
	...
	if (bFirst) {		// param_k=PARAM_Kbit
		...
		std::fill_n(tSkle->iKb, DATA_NUMBER_LENGTH, 0);
		tSkle->iKb[0] = 1;
	}

	// SCI
	SCI(cM, cD, tSkle->cSb[0], tSkle->cSb[1], tSkle->iKb, tPre, idx, tRecv);
	#ifdef _DEBUG_SkLE_s
	//DebugOut("S = Sum_(i=1~n) P[i]", tOut->cTotal_S.c, idx);
	DebugDec("M = \t\t\t\t", cM, idx);
	DebugDec("D = \t\t\t\t", cD, idx);
...
////	iCmpRes = Comparison(&(tOut->cCmp), &(tOut->cTotal_S), tSkle->cSb[0], tSkle->cSb[1], iTmp, tSkle->iKb, tPre, idx, tRecv);
////	tOut->iCmp = Comparison(&(tOut->cCmp), &(tOut->cTotal_S), tSkle->cSb[0], tSkle->cSb[1], iTmp, tSkle->iKb, tPre, idx, tRecv);
//	#ifdef _DEBUG_SkLE_s
...
//	cD = &(tSkle->cD);
	...
	for (int i=0 ; i<iNum ; i++) {
		//iTidx = bFirst ? tOut->iRan[idx]+i  : tOut->iRan2[idx]+i;
		cE = bFirst ? &(tOut->cDisB[i][bit]) : &(tOut->cFbit[i][bit]);
		cC = &(tSkle->cC[i]);
		cK = &(tSkle->cK[i]);
		cU = &(tSkle->cU[i]);
...
void PaillierCrypto::SkLE_s_5(out_t* tOut, skle_t* tSkle, bool bFirst, unsigned short idx)
{
	...
	for (int i=0 ; i<iNum ; i++) {			// i = 1 ~ n
		// K[i] = K[i] * C[i]
		paillier_mul(mPubKey, (tOut->cK)+SrtDat+i, (tSkle->cK)+i, (tSkle->cC)+i);
		#ifdef _DEBUG_SkLE_s
